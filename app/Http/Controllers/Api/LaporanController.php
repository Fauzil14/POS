    ...
    public function laporanStokBarang($waktu) 
    {
        ...
        switch (strlen($waktu)) {
            ...
            case 4 : // year
                ...
                $keluar = $keluar->map(function($item, $key) {
                    $new = array_merge([ 'bulan' => Carbon::parse($key)->translatedFormat('F') ], $this->processStokBarang($item, null));
                    return $new;
                })->values()->all();

                    $masuk = $masuk->map(function($item, $key) {
                        $new = array_merge([ 'bulan' => Carbon::parse($key)->translatedFormat('F') ], $this->processStokBarang(null, $item));
                        return $new;
                    })->values()->all();
